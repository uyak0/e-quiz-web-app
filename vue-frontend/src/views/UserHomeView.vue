...
<template>
  <TopBar @modal-enabled="modalEnabled = true" v-model:enable-button="joinOrCreateClassroomBtn"/>
  ...
  <div v-else class="flex justify-center">
    <div class="sm:w-3/4 w-full flex flex-col sm:flex-row sm:flex-wrap content-start">
      ...
      <button @click="modalEnabled = true" v-if="classrooms.length && userRole === 'student'" class="cursor-pointer hover:border-gray-100 hover:text-gray-100 ease-in-out duration-500 my-4 mx-4 rounded-md border-dashed border-2 border-gray-500 bg-transparent text-gray-500 h-48 sm:w-48 overflow-hidden w-full">
      ...
      <!-- Create Classroom -->
      <button @click="modaEnabled = true" v-else-if="classrooms.length && userRole === 'teacher'" class="hover:border-gray-100 hover:text-gray-100 ease-in-out duration-500 my-4 mx-4 rounded-md border-dashed border-2 border-gray-500 bg-transparent text-gray-500 h-48 sm:w-48 overflow-hidden w-full">
        <div class="w-full h-full text-2xl flex flex-col justify-items-center justify-center">
          <p class="place-self-center font-bold">+</p>
  ...
  <Modal v-if="userRole==='student'" v-model="modalEnabled">
    <div class="bg-gray-600 rounded-md w-3/4 h-fit text-2xl flex place-self-center z-20">
